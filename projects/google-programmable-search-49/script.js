...
// config
var lQModePrint = `<a class="brand light irvTag2 itemHeight border borderRadius2 small" href="./?q=">start</a>`;
...
lQModePrint = "";
lQModePrint = `<a class="brand light irvTag2 itemHeight border borderRadius2 small" href="./?q=">start</a>`;

lQMode.forEach((item, item2) => {
if (item["comName"] == qGMode){
lQModePrint += `
<a class="brand active2 light3 irvTag2 itemHeight border borderRadius2 small" title="${item["title"]}" href="?mode=${item["title"]}&q=${qGQ}">${item["name"]}</a>
`;
} else {
lQModePrint += `
<a class="brand irvTag2 itemHeight light3 border borderRadius2 small" title="${item["title"]}" href="?mode=${item["comName"]}&q=${qGQ}">${item["name"]}</a>
`;
}
});
...
if (item["comName"] == qGMode){
lQModePrint += `
<a class="brand active2 light3 irvTag2 itemHeight border borderRadius2 small" title="${item["title"]}" href="?mode=${item["comName"]}&q=${qGQ}">${item["name"]}</a>
`;
} else {
lQModePrint += `
<a class="brand irvTag2 light2 border borderRadius2 small" title="${item["title"]}" href="?mode=${item["comName"]}&q=${qGQ}">${item["name"]}</a>
`;
}
});
document.getElementById("mode").innerHTML = `${lQModePrint}`; 
...
<a class="irvTag2 borderBottomBlue blue small notUnderline" title="${item["title"]}" href="https://cse.google.com/cse?cx=${item["code"]}">Public URL</a>
`;
